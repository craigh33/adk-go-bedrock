...
- **Token usage**: Bedrock input, output, total and cache-read counts map to `genai` `UsageMetadata`. Cache writes have no `genai` equivalent, so the Bedrock count is surfaced as `CustomMetadata["bedrock_cache_write_input_tokens"]` (`int32`)
