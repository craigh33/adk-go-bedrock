<p align="center">
  <img
    ...
    alt="adk-go-bedrock banner showing Agent Development Kit connected to Amazon Bedrock"
    width="100%"
  />
</p>
...
- **Request-side guardrails**: Use [`bedrock.GuardrailConfig`](bedrock/guardrail.go) with [`bedrock.New`](bedrock/converse.go)’s `Options.Guardrail`, [`bedrock.WithGuardrail`](bedrock/modeloptions.go), and/or per-request [`bedrock.ContextWithGuardrail`](bedrock/guardrail.go) to set Bedrock `GuardrailIdentifier` and `GuardrailVersion` (and optional `Trace`). That is the supported path; genai `SafetySettings` and `ModelArmorConfig` are not mapped to Bedrock. If the request includes those genai fields and **no** explicit Bedrock guardrail is in effect, the request builder still returns an error instead of dropping them silently. When a Bedrock guardrail *is* in effect, those genai fields are ignored for this provider.
