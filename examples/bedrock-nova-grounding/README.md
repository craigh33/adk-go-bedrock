...
- **Bedrock region** (for example `AWS_REGION=us-west-2`) — Web Grounding for Nova is documented for US regions. For current regional availability, see [Amazon Bedrock pricing](https://aws.amazon.com/bedrock/pricing/). For testing purposes, us-west-2 region and inference parameters work.
- **`BEDROCK_MODEL_ID`** — an inference profile that supports Web Grounding with your account (default in code: `us.amazon.nova-2-lite-v1:0`). Confirm current IDs in [Amazon Nova Web Grounding](https://docs.aws.amazon.com/nova/latest/userguide/grounding.html).
